...
static
void s_throw_image_exception (MagickWand *magick_wand, const char *message)
{
	...
	if (magick_msg && strlen (magick_msg) > 0) {
		zend_throw_exception(php_zbarcode_exception_class_entry, magick_msg, 1);
	...
	zend_throw_exception(php_zbarcode_exception_class_entry, message, 1);
}
...
PHP_METHOD(zbarcodeimage, __construct)
{
	...
	if (!tsrm_realpath(filename->val, resolved_path)) {
		zend_throw_exception(php_zbarcode_exception_class_entry, "The file does not exist or cannot be read", 1);
		return;
	}
	...
	if (!s_php_zbarcode_read(intern->magick_wand, resolved_path, enhance)) {
		s_throw_image_exception (intern->magick_wand, "Unable to read the image");
		return;
	}
	return;
}
...
static
zval *s_php_zbarcode_scan_page(zbar_image_scanner_t *scanner, zbar_image_t *image, zend_bool extended, zval *return_array)
{
	int n;
...
PHP_METHOD(zbarcodescanner, scan)
{
	...
	if (Z_TYPE_P(image) != IS_OBJECT && Z_TYPE_P(image) != IS_RESOURCE) {
		zend_throw_exception(php_zbarcode_exception_class_entry, "The first parameter must be a valid object or a GD resource", 1);
		return;
	}
#else
	if (Z_TYPE_P(image) != IS_OBJECT) {
		zend_throw_exception(php_zbarcode_exception_class_entry, "The first parameter must be a valid object", 1);
		return;
	}
#endif
	...
	if (Z_TYPE_P(image) == IS_OBJECT && instanceof_function_ex(Z_OBJCE_P(image), php_zbarcode_image_sc_entry, 0)) {
...
#ifdef HAVE_ZBARCODE_IMAGICK	
	else if (Z_TYPE_P(image) == IS_OBJECT && instanceof_function_ex(Z_OBJCE_P(image), php_imagick_get_class_entry(), 0)) {
		php_zbarcode_image_object *intern_image;
		intern_image = (php_zbarcode_image_object *)Z_ZBARCODE_IMAGE_OBJ_P(image);
		magick_wand  = intern_image->magick_wand;
	}
	...
	/* Handle GD resource */
		...
		if (!gd_image) {
			zend_throw_exception(php_zbarcode_exception_class_entry, "The given resource is not a valid GD image", 1);
			return;
		}
		...
		if (!has_pixels) {
			efree(pixels);
			zend_throw_exception(php_zbarcode_exception_class_entry, "Unable to get pixels from GD resource", 1);
			return;
		}
		...
		if (!wand) {
			efree(pixels);
			zend_throw_exception(php_zbarcode_exception_class_entry, "Failed to allocate MagickWand structure", 1);
			return;
		}
		...
		if (!color) {
			...
			zend_throw_exception(php_zbarcode_exception_class_entry, "Failed to allocate PixelWand structure", 1);
			return;
		}
		...
		zend_throw_exception(php_zbarcode_exception_class_entry, "Invalid parameter type", 1);
		return;
	}
	...
	if (image_count == 0) {
		zend_throw_exception(php_zbarcode_exception_class_entry, "The image object does not contain images", 1);
		return;
	}
	...
	if (page_num > 0) {
		if (image_count < page_num) {
			...
			zend_throw_exception_ex(php_zbarcode_exception_class_entry, 1, "Invalid page specified. The object contains %ld page(s)", image_count);
			return;
		}
		...
		if (MagickSetIteratorIndex(magick_wand, page_num - 1) == MagickFalse) {
			...
			zend_throw_exception(php_zbarcode_exception_class_entry, "Failed to set the page number", 1);
			return;
		}
		/* Read page */
		...
		if (!page) {
			...
			zend_throw_exception(php_zbarcode_exception_class_entry, "Failed to get the page", 1);
			return;
		}

		/* Scan the page for barcodes */
		s_php_zbarcode_scan_page(intern->scanner, page, extended, return_value);
	} else {
		...
		while (MagickNextImage(magick_wand) != MagickFalse) {
			...
			s_php_zbarcode_scan_page(intern->scanner, page, extended, &page_array);
...
PHP_METHOD(zbarcodescanner, setconfig)
{
	...
	if (zbar_image_scanner_set_config(intern->scanner, symbology, name, value) != 0) {
		zend_throw_exception(php_zbarcode_exception_class_entry, "Config does not apply to specified symbology, or value out of range", 1);
		return;
	}
	PHP_ZBARCODE_CHAIN_METHOD;
}
...
PHP_MINIT_FUNCTION(zbarcode)
{
	...
	*/
	...
	php_zbarcode_exception_class_entry = zend_register_internal_class_ex(&ce, zend_exception_get_default());
	...
	*/
	...
	php_zbarcode_sc_entry = zend_register_internal_class(&ce);
	...
	*/
	...
	php_zbarcode_image_sc_entry = zend_register_internal_class(&ce);
	...
	*/
	...
	php_zbarcode_scanner_sc_entry = zend_register_internal_class(&ce);

	/* Do we have imagick support */
#ifdef HAVE_ZBARCODE_IMAGICK
	zend_declare_class_constant_bool(php_zbarcode_sc_entry, "HAVE_IMAGICK", sizeof("HAVE_IMAGICK")-1, 1);
#else 
	zend_declare_class_constant_bool(php_zbarcode_sc_entry, "HAVE_IMAGICK", sizeof("HAVE_IMAGICK")-1, 0);
#endif

	/* Do we have GD support */
#ifdef HAVE_ZBARCODE_GD
	zend_declare_class_constant_bool(php_zbarcode_sc_entry, "HAVE_GD", sizeof("HAVE_GD")-1, 1);
#else 
	zend_declare_class_constant_bool(php_zbarcode_sc_entry, "HAVE_GD", sizeof("HAVE_GD")-1, 0);
#endif

#define PHP_ZBARCODE_REGISTER_CONST_LONG(const_name, value) \
	zend_declare_class_constant_long(php_zbarcode_sc_entry, const_name, sizeof(const_name)-1, (long)value);
